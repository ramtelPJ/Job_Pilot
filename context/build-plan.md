...
- Create instrumentation-client.ts at the project root — PostHog browser init (`posthog-js`), guarded by NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN presence. Not a `lib/posthog-client.ts` file and not a React provider — this is the Next.js 15.3+ file convention, and `posthog.init()` needs no wrapping component
- Create lib/posthog-server.ts — `getPostHogClient()`, a `posthog-node` singleton with flushAt: 1 and flushInterval: 0, returning `null` when unconfigured
- `components/analytics/IdentifyUser.tsx` — Client Component in the root layout, fed by a server-side `getCurrentUser()` call, calls `posthog.identify(userId, { email })` — covers login and "already logged in on refresh" in one place since sign-in is entirely server-initiated (no client-side auth code to hook `identify()` into directly)
- `components/analytics/ResetAnalytics.tsx` — calls `posthog.reset()`, rendered on `/login?loggedOut=1` (the `signOut()` Server Action's redirect target)
...
Extract from Resume button — Claude reads uploaded PDF and auto-fills profile form fields.
...
- Claude reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using Claude.

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- Claude generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders Claude output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - Claude scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from Claude
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to Claude synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **Claude synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, Claude synthesizes from job description and profile alone.
