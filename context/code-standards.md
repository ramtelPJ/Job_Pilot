...
| `sign_in_initiated`   | OAuth sign-in button submitted (Server Action) | provider                   |
| `sign_in_completed`   | OAuth callback exchanged the code successfully | provider (server-side, identified) |
| `sign_in_failed`      | OAuth callback failed (missing code or exchange error) | reason (anonymous — no user yet) |
| `sign_out`            | User signs out                              | (identified with the outgoing user's id before session clears) |
| `cta_clicked`         | A marketing CTA link is clicked (Navbar, Hero, CTASection) | cta, location  |
...
**Auth events run server-side** (`posthog-node`, via `lib/posthog-server.ts`'s `getPostHogClient()`) since sign-in/out are entirely Server Action/Route Handler driven — never client `posthog-js` calls. `sign_in_failed` has no known user yet, so it's captured with a random anonymous `distinctId` (never `null`/a shared literal) rather than skipped. `cta_clicked` is the only client-side (`posthog-js`) event so far, fired from `components/analytics/TrackedLink.tsx`.
...
| Variable                        | Used In                                    |
| ------------------------------- | ------------------------------------------ |
| `NEXT_PUBLIC_INSFORGE_URL`      | lib/insforge-client.ts, lib/insforge-server.ts (default env resolution, no explicit config needed) |
