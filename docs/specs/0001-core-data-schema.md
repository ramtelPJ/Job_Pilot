...
- [x] Confirmed (Feature 06): a private bucket's object URL rejects any request with no bearer token (`AUTH_INVALID_CREDENTIALS`) — a plain `<a href>` link 401s, so `ResumeUpload` now fetches via `insforge.storage.from('resumes').download(path)` (the authenticated SDK client) and opens a blob URL instead. Still open: whether one signed-in user's own token can successfully download a DIFFERENT user's `resume.pdf` path (path-scoped ownership, not just "authenticated at all"). Worth a deliberate test with two real accounts before this matters for anything more sensitive than a resume.
...
- [x] Confirmed (Feature 16): `jobs.found_at` records when a job was *discovered* via Adzuna search, not when it was researched — using it for the Recent Activity feed's "Researched {company} — {time ago}" entries would show the wrong timestamp (the search time, not the research time), since research can happen long after a job is first found. Added `jobs.researched_at timestamptz`, nullable, plus a matching `idx_jobs_researched_at` index (same shape as the existing `idx_jobs_found_at`), via a direct additive migration (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`) rather than a full `/architect` pass — a single nullable, non-breaking column with no cascade or constraint implications. Set alongside `company_research` in `app/api/agent/research/route.ts`'s update call.
