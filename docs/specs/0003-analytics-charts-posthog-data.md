...
- [ ] Once `POSTHOG_PERSONAL_API_KEY` is added for real, confirm `lib/posthog-query.ts`'s parsing of the `results` field against an actual response (its exact shape, array of row arrays vs. named columns, was not fully confirmed from PostHog's docs at spec time) and tighten the parser if the defensive fallback path ever actually triggers
